...
class Bomb:
    ...
    def explosion(self, map, box, bombs, hero, enemy, explosion):
        ...
        for i in range(self.X, min(self.X + self.r + 1, settings.WIDTH - settings.ICON + 1), settings.ICON):
            ...
            if x is not None or (type(y) == int and (y == 1 or y == 2)):
                explosion.append(Explosion(i, self.Y))
            if x is None and type(y) == int and y == 3:
                break
            if x is None and type(y) == int and y == 4:
                if self.type == 1:
                    hero.score += 1
                else:
                    enemy.score += 1
        ...
        for i in range(self.X - settings.ICON, max(-1, self.X - self.r - 1), -settings.ICON):
            ...
            if x == None and type(y) == int and y == 2:
                enemy.alive = False
        for i in range(settings.ICON + self.Y, min(self.Y + self.r + 1, settings.HEIGHT - settings.ICON + 1),
                       settings.ICON):
            ...
            if x is None and type(y) != int:
                bombs[bombs.index(y)].start = 7.0
                break
            if x is None and type(y) == int and y == 1:
                hero.alive = False
            if x == None and type(y) == int and y == 2:
                enemy.alive = False
